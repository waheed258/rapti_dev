...
public partial class User_CashBookType : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            else
            {
                ...
                qs = objBALglobal.Decrypts(HttpUtility.UrlDecode(getId), true);
    ...
    private void InsertUpdateCash()
    {
        try
        {
            ...
            objEMCash.BranchId = 1;
            objEMCash.CompanyId = 1;

            int Result = objBACash.InsUpdCashBook(objEMCash);
            if (Result > 0)
            {
                lblMsg.Text = objBALglobal.ShowMessage("success", "Success", "CashBook Types Created Successfully");
            ...
            {
                lblMsg.Text = objBALglobal.ShowMessage("info", "Info", "CashBook Types  was not created please try again");
            }
        }
        catch (Exception ex)
        {
    ...
    void ClearControls()
    {
        ...
        dropDefaultAction.SelectedValue = "0";
    }
